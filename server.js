...
// Use global fetch available in Node >= 18. If you need node-fetch, install it separately.
...
const firebaseConfig = {
    databaseURL: process.env.FIREBASE_DB_URL || "https://jal-mahakal-shakti-default-rtdb.asia-southeast1.firebasedatabase.app"
};
...
// Public runtime configuration for frontend (no secrets)
...
// 🔐 DEVICE TOKEN (simple auth) - override with env var in production
const DEVICE_TOKEN = process.env.DEVICE_TOKEN || "JMS_DEVICE_001";

// 🔥 FIREBASE DETAILS - provide via env vars in production
const FIREBASE_DB_URL = process.env.FIREBASE_DB_URL || "https://jal-mahakal-shakti-default-rtdb.asia-southeast1.firebasedatabase.app";
const FIREBASE_DB_SECRET = process.env.FIREBASE_DB_SECRET || process.env.FIREBASE_AUTH || "";

if (!process.env.FIREBASE_DB_URL) {
        console.warn('⚠️ FIREBASE_DB_URL is not set. Using default value from source.');
}
if (!process.env.FIREBASE_DB_SECRET) {
        console.warn('⚠️ FIREBASE_DB_SECRET is not set. It is recommended to provide a secure credential via env vars.');
}
  ...
  try {
    ...
    const payload = {
      ...
      timestamp: Date.now()
        ...
        // 🔗 Firebase REST URL (built from env or default)
        let url = (process.env.FIREBASE_DB_URL || 'https://jal-mahakal-shakti-default-rtdb.asia-southeast1.firebasedatabase.app') + '/pipeline/node01.json';
        const secret = process.env.FIREBASE_DB_SECRET || process.env.FIREBASE_AUTH || '';
...
// Removed duplicate `app.listen(3000)` to avoid EADDRINUSE when
// the HTTP server is created later with `http.createServer(app)`
// and `server.listen(...)` (see bottom of this file).
// ==================== CLEANUP ON EXIT ====================
